...
public class OrderServiceImpl implements OrderService {
    ...
    public List<Order> findOrdersNotSend() {
        ...
        Example.Criteria criteria = example.createCriteria();
        criteria.andEqualTo("payStatus", 1);//1是已支付
        criteria.andEqualTo("orderStatus", 1);//1是待发货
    ...
    public void orderTimeOutLogic() {
        ...
        //获得超时时间
        LocalDateTime localDateTime = LocalDateTime.now().minusMinutes(orderTimeout);//往前推时间-60分钟，只要前面创建的时间还小于推过去的时间说明超时
        Example example=new Example(Order.class);
        Example.Criteria criteria = example.createCriteria();
      criteria.andLessThan("createTime",localDateTime);//超时的
