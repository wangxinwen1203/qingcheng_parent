...
public class PreferentialServiceImpl implements PreferentialService {
    ...
    public int findPreMoneyByCategoryId(Integer categoryId, int money) {
        ...
        //指定查询条件在优惠规则计算表中查询
        //查询条件：状态1    分类：消费金额，开始时间和截止时间 排序：消费额排序
...
//        example.orderBy("buyMoney").desc()
        ...
        if (preferentialList.size() >= 1) {//有优惠
            Integer preMoney = preferentialList.get(0).getPreMoney();
            if ("1".equals(preferentialList.get(0).getType())) {//不翻倍

            } else {//翻倍
                preMoney = money / preferentialList.get(0).getBuyMoney() * preMoney;
            }
            return preMoney;
        } else {
            return 0;
