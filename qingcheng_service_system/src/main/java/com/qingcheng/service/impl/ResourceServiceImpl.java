...
public class ResourceServiceImpl implements ResourceService {
    ...
    private List<Map> findByParentId(List<Resource> resources, Integer parentId) {

        List<Map> mapList = new ArrayList<Map>();
        for (Resource resource : resources) {
            if (resource.getParentId().equals(parentId)) {
                Map map = new HashMap();
                map.put("id", resource.getResKey());
                map.put("resKey", resource.getResKey());
                map.put("resName", resource.getResName());

                map.put("children", findByParentId(resources, resource.getId()));
                mapList.add(map);
            }
        }
        return mapList;
    ...
    //根据用户名去查询其资源权限
    public List<String> findResourceByName(String name) {
        List<String> allResKey = resourceMapper.findAllResKey(name);
