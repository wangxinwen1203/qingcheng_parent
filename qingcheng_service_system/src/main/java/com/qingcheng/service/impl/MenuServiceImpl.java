...
public class MenuServiceImpl implements MenuService {
    ...
    public List<Map> findAllMenu() {
        ...
        return findMapById(list,"0");
    }
    public List<Map> findMapById(List<Menu> menuList,String parentId){
        List<Map> mapList=new ArrayList<Map>();
        for (Menu menu : menuList) {
            if(menu.getParentId().equals(parentId)){
                HashMap hashMap = new HashMap();
                hashMap.put("path",menu.getId());
                hashMap.put("title",menu.getName());
                hashMap.put("icon",menu.getIcon());
                hashMap.put("linkUrl",menu.getUrl());
                hashMap.put("children",findMapById(menuList,menu.getId()));
                mapList.add(hashMap);
            }
        }
        return mapList;
